...
from ClaseFraccion import claseFraccion
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='/', command=partial(self.ponerFraccion, '/')).grid(column=3, row=7, sticky=W)
        ...
        ttk.Button(mainframe, text='C', command=self.borrarPanel).grid(column=1, row=8, sticky=W)
        self.__panel.set('')
        panelEntry.focus()
    ...
    def ponerNUMERO(self, numero):
        if self.__operadorAux==None:
            ...
            self.__operadorAux=None
            valor=self.__panel.get()
            if (self.__fraccion==False):
                self.__primerOperando=int(valor)
            else:
                self.__primerOperando=claseFraccion(valor)
            self.__panel.set(numero)
    
    def ponerFraccion(self, numero):
        if self.__operadorAux==None:
            valor = self.__panel.get()
            self.__panel.set(valor+numero)
        else:
    ...
    def ponerOPERADOR(self, op):
        if op=='=':
            ...
            if (self.__fraccion==False):
                ...
                self.__segundoOperando=claseFraccion(self.__panel.get())
